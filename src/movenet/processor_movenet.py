...
class ProcessorMovenet:
    ...
    def __get_keypoints_movenet(self,input_image, model_path,width,height,dtype):
        print(model_path)
    ...
    def process(self,image_path:str, model_ps:PoseEstimationModel):
        ...
        if model_ps == PoseEstimationModel.MOVENET_LIGHTNING_V4_MULTIPOSE or model_ps == PoseEstimationModel.MOVENET_THUNDER_V4_SINGLEPOSE:
            self.keypoints = self.__get_keypoints_movenet_v4(image_path,model_ps.movenet_model)
        if model_ps == PoseEstimationModel.MOVENET_LIGHTNING_INT_8 or model_ps == PoseEstimationModel.MOVENET_LIGHTNING_FLOAT_16:
        ...
        if model_ps == PoseEstimationModel.MOVENET_LIGHTNING_V4_8 or model_ps == PoseEstimationModel.MOVENET_LIGHTNING_V4_16:
            keypoints = self.__get_keypoints_movenet(image_path,model_ps.movenet_model,192,192,tf.uint8)
        if model_ps == PoseEstimationModel.MOVENET_THUNDER_INT_8:
